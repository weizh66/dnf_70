...
function checkExecutableSkill_IceSwordnew(obj)
{
	...
	local b_useskill = obj.sq_IsUseSkill(SKILL_ICE_SWORDNEW);
	if(b_useskill) {		
		obj.sq_AddSetStatePacket(STATE_ICE_SWORDNEW, STATE_PRIORITY_USER, false);
		return true;
	}		
	return false;
}
...
function checkCommandEnable_IceSwordnew(obj)
{
	...
	if(state == STATE_ATTACK) {
		return obj.sq_IsCommandEnable(SKILL_ICE_SWORDNEW);
	} 
...
function onSetState_IceSwordnew(obj, state, datas, isResetTimer)
{
	...
	local damage = obj.sq_GetBonusRateWithPassive(SKILL_ICE_SWORDNEW , STATE_ICE_SWORDNEW, 0, 1.0);
	...
	local skillLevel = sq_GetSkillLevel(obj, SKILL_ICECHAKRAM);
	if (skillLevel > 0)
	{
...
function onKeyFrameFlag_IceSwordnew(obj, flagIndex)
{
	...
	else if (flagIndex == 2)
	{
		...
		local skill_level	= sq_GetSkillLevel(obj, SKILL_ICE_SWORDNEW);
		local damage = obj.sq_GetBonusRateWithPassive(SKILL_ICE_SWORDNEW , STATE_ICE_SWORDNEW, 1, 1.0);
		obj.sq_SetCurrentAttackBonusRate(damage);	
		...
		local ice_power	= sq_GetLevelData(obj, SKILL_ICE_SWORDNEW, 2, skill_level);
		local ice_prob	= sq_GetLevelData(obj, SKILL_ICE_SWORDNEW, 3, skill_level);
		local ice_time	= sq_GetLevelData(obj, SKILL_ICE_SWORDNEW, 4, skill_level);
