...
function checkExecutableSkill_new(obj)
{
	if (!obj) return false;
	local isUse = obj.sq_IsUseSkill(SKILL_new);
	...
	if (isUse) {
		...
		obj.sq_AddSetStatePacket(STATE_new , STATE_PRIORITY_USER, false);
		return true;
	}
...
function checkCommandEnable_new(obj)
{
	if (!obj) return false;
	...
	{
		return obj.sq_IsCommandEnable(SKILL_new);
		obj.sq_IsCommandEnable(SKILL_new);
	}	
...
function onSetState_new(obj, state, datas, isResetTimer)
{	
	if(!obj) return;
	...
	if(substate == 3)
	{
		...
		obj.sq_SetCurrentAttackInfo(32);
	}	
	else if(substate == 4)
	{
...
function onEndCurrentAni_new(obj)
{
	local substate = obj.getSkillSubState();//?閫哀ubstate
	if(substate == 0){
		obj.getVar().clear_vector();
		obj.getVar().push_vector(1);
		obj.sq_AddSetStatePacket(STATE_new, STATE_PRIORITY_IGNORE_FORCE, true);
	}
	if(substate == 1){
		obj.getVar().clear_vector();
		obj.getVar().push_vector(2);
...
function onKeyFrameFlag_new(obj,flagIndex)
{
	if(!obj)
		return false;
