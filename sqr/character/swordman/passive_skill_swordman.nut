...
function ProcPassiveSkill_swordman(obj, skill_index, skill_level)
{
	if (skill_index == SKILL_EXPRESSION)
	{
		if(skill_level > 0)
		{
			local appendage = CNSquirrelAppendage.sq_AppendAppendage(obj, obj, skill_index, false, "Character/swordman/Expressions/ap_swordman_expression.nut", true);
	...
	{ // 불사
		if(skill_level > 0)
		{
			...
			"Character/swordman/DieHard/ap_swordman_DieHard.nut", true);
...
function onUseSkillPassiveSkill_swordman(obj,skillIndex, skillLevel)
{
	if(!obj)
		return;		
...
function ProcPassiveSkill_Swordman(obj, skill_index, skill_level)
{
	if (skill_index == SKILL_FORCEOFTHUNDER)
	{
		if(skill_level > 0)
		{
			local appendage = CNSquirrelAppendage.sq_AppendAppendage(obj, obj, skill_index, false, "Character/Swordman/forceofthunder/ap_forceofthunder.nut", true);
		}
	}
	return true;
}
