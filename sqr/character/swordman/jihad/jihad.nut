...
{
	if (!obj) return false;
	local isUse = obj.sq_IsUseSkill(SKILL_jihad);
	...
	if (isUse) {
		...
		obj.sq_AddSetStatePacket(STATE_jihad , STATE_PRIORITY_USER, false);
		return true;
	}
...
function checkCommandEnable_jihad(obj)
{
	...
	{
		return obj.sq_IsCommandEnable(SKILL_jihad);
		obj.sq_IsCommandEnable(SKILL_jihad);
	}	
...
{	
	...
	if(substate == 0)
	{
		obj.sq_SetCurrentAnimation(CUSTOM_ANI_jihad);
		obj.sq_SendCreatePassiveObjectPacket(234501, 0, 200, 0, 0);
	}
...
function onKeyFrameFlag_jihad(obj,flagIndex)
{
	if(!obj)
		return false;
