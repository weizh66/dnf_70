...
{
	if (!obj) return false;
	local isUse = obj.sq_IsUseSkill(SKILL_ragnarok);
	...
	if (isUse) {
		...
		obj.sq_AddSetStatePacket(STATE_ragnarok , STATE_PRIORITY_USER, false);
		return true;
	}
...
function checkCommandEnable_ragnarok(obj)
{
	...
	{
		return obj.sq_IsCommandEnable(SKILL_ragnarok);
		obj.sq_IsCommandEnable(SKILL_ragnarok);
	}	
...
{	
	...
	if(substate == 0)
	{
		...
		obj.sq_SendCreatePassiveObjectPacket(250223, 0, 0, 0, 0);
